# HireCode AI (Qwen3-32B-AWQ)

HireCode AI — демонстрационная платформа техсобеседований с живым AI-интервьюером. Полностью русифицированный интерфейс: все задачи, ответы AI и интерфейс на русском языке. Архитектура полностью повторяет боевую систему: FastAPI + PostgreSQL + Redis + Docker runner + React 18 / Vite / Tailwind / shadcn UI.
...
- AI-интервьюер (Qwen3-32B-AWQ) на едином OpenAI совместимом endpoint (стриминг через WebSocket, комментарии к действиям кандидата).
...
- Единственный клиент OpenAI совместимого API с моделью Qwen3-32B-AWQ: `OPENAI_API_KEY`, `OPENAI_BASE_URL` (по умолчанию `https://llm.t1v.scibox.tech/v1`), `OPENAI_MODEL`.
- Все ответы AI-интервьюера строго на русском языке, все задачи на русском языке.
